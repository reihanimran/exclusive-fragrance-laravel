<x-app-layout>
    ...
    <div class="relative w-full h-[40vh] flex items-center justify-center">
        <div class="absolute inset-0">
            <img src="{{ asset('uploads/banners/shop-bg.jpg') }}" alt="Luxury Fragrance Collection"
        ...
        <div class="relative z-10 text-center px-4">
            ...
            <p class="text-xl text-[#F5D57A] max-w-2xl mx-auto">Discover our curated selection of premium scents crafted
    ...
    <div class="container mx-auto px-4 py-8 bg-[#151E25]">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <button id="filterToggle"
                class="lg:hidden bg-[#F5D57A] text-[#151E25] px-4 py-2 rounded-md mb-4 flex items-center justify-center">
                ...
                <form id="filterForm" method="GET" action="{{ route('shop.index') }}">
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center cursor-pointer"
                            ...
                            <svg id="categories-arrow" class="w-6 h-6 text-[#F5D57A] transition-transform" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center cursor-pointer"
                            ...
                            <svg id="fragrance-arrow" class="w-6 h-6 text-[#F5D57A] transition-transform" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                                ...
                                @foreach($filterData['fragranceTypes'] as $type)
                                    <label class="flex items-center text-white">
                                        <input type="checkbox" name="fragrance_type[]" value="{{ $type }}"
                                            class="mr-2 accent-[#F5D57A]" @if(in_array($type, request('fragrance_type', []))) checked @endif>
                                        {{ $type }}
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center cursor-pointer" onclick="toggleAccordion('size')">
                            ...
                            <svg id="size-arrow" class="w-6 h-6 text-[#F5D57A] transition-transform" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center cursor-pointer"
                            ...
                            <svg id="gender-arrow" class="w-6 h-6 text-[#F5D57A] transition-transform" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                                ...
                                @foreach($filterData['genders'] as $gender)
                                    <label class="flex items-center text-white">
                                        <input type="checkbox" name="gender[]" value="{{ $gender }}"
                                            class="mr-2 accent-[#F5D57A]" @if(in_array($gender, request('gender', [])))
                                            checked @endif>
                                        {{ $gender }}
                                    </label>
                                @endforeach
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center cursor-pointer"
                            ...
                            <svg id="stock-arrow" class="w-6 h-6 text-[#F5D57A] transition-transform" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center text-white">
                                    <input type="checkbox" name="stock_status" value="in_stock"
                                        class="mr-2 accent-[#F5D57A]" @if(request('stock_status') === 'in_stock') checked
                                        @endif>
                                    In Stock
                                </label>
                                <label class="flex items-center text-white">
                                    <input type="checkbox" name="bestseller" value="1" class="mr-2 accent-[#F5D57A]"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="sort"
                            class="w-full px-3 py-2 bg-[#151E25] border border-gray-600 rounded text-white">
                            <option value="latest" @if(request('sort', 'latest') === 'latest') selected @endif>Newest
                                Arrivals</option>
                            <option value="price_asc" @if(request('sort') === 'price_asc') selected @endif>Price: Low to
                                High</option>
                            <option value="price_desc" @if(request('sort') === 'price_desc') selected @endif>Price: High
                                to Low</option>
                            <option value="name_asc" @if(request('sort') === 'name_asc') selected @endif>Name: A to Z
                            </option>
                            <option value="name_desc" @if(request('sort') === 'name_desc') selected @endif>Name: Z to A
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="flex space-x-4 mt-6">
                        <button type="submit"
                            class="flex-1 bg-[#F5D57A] hover:bg-[#e0c060] text-[#151E25] py-2 px-4 rounded-md transition-colors">
                            Apply Filters
                        </button>
                        <a href="{{ route('shop.index') }}"
                            class="flex-1 bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded-md text-center transition-colors">
                            Reset
                        </a>
                    </div>
                </form>
            ...
            <div class="w-full lg:w-3/4">
                ...
                <div class="bg-[#1e293b] rounded-lg p-4 mb-6 flex flex-col md:flex-row justify-between items-center">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <select class="bg-[#151E25] border border-gray-600 rounded text-white px-3 py-2"
                            ...
                            <div class="border border-[#F5D57A] rounded-lg p-4 flex flex-col items-center">
                                <div class="relative">
                                    <a href="{{ route('shop.show', $product->id) }}">
                                        <img src="{{ $product->featuredImage?->image_path ? asset('storage/' . $product->featuredImage->image_path) : asset('uploads/images/default-placeholder.png') }}"
                                            alt="{{ $product->product_name }}" class="w-full object-cover">
                                    </a>
                                    @if($product->stock_quantity <= 0)
                                        <div class="absolute top-4 right-4 bg-red-600 text-white text-xs px-2 py-1 rounded">Sold Out
                                        </div>
                                ...
                                <div class="py-4 px-2 self-start w-full">
                                    <div class="flex flex-col justify-between items-start mb-2">
                                        <a href="{{ route('shop.show', $product->id) }}"
                                            class="text-white font-semibold hover:text-[#F5D57A] transition-colors">{{ $product->product_name }}</a>
                                        <span class="text-[#F5D57A] font-bold">LKR
                                            {{ number_format($product->sale_price ?? $product->original_price, 2) }}</span>
                                    </div>
                ...
                @else
                    <div class="text-center text-white py-12">
                        ...
                        <p class="text-gray-400">Try adjusting your filters or browse other categories.</p>
                    </div>
                @endif
            </div>
        </div>
