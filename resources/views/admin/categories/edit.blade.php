                ...
                <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-8">
                        ...
                        @if($category->featured_img)
                            <div id="current-image" class="mb-4 p-4 bg-gray-50 rounded-lg border">
                                <p class="text-sm text-gray-600 mb-3">Current Image:</p>
                                <img src="{{ asset('storage/' . $category->featured_img) }}"
                            ...
                            <div id="upload-content" class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="featured_img" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500">
                                        <span>{{ $category->featured_img ? 'Replace with new file' : 'Upload a file' }}</span>
                                        <input type="file" 
                                               name="featured_img" 
                                               id="featured_img" 
                                               class="sr-only"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fileInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    displayFile(file);
                    // Uncheck remove image checkbox if new file is selected
            ...
            if (removeImageCheckbox && currentImageDiv) {
                removeImageCheckbox.addEventListener('change', function() {
                    if (this.checked) {
                        currentImageDiv.style.opacity = '0.5';
                        // Clear any selected new file
                        fileInput.value = '';
                        hideFile();
                    } else {
