<x-admin-layout>
    ...
    <x-slot name="header">Edit Product: {{ $product->product_name }}</x-slot>
            ...
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <select id="fragrance_type" name="fragrance_type" required
                            ...
                            <option value="Woody Aromatic" {{ (old('fragrance_type', $product->fragrance_type) == 'Woody Aromatic') ? 'selected' : '' }}>Woody Aromatic</option>
                            <option value="Floral" {{ (old('fragrance_type', $product->fragrance_type) == 'Floral') ? 'selected' : '' }}>Floral</option>
                            <option value="Fresh Spicy" {{ (old('fragrance_type', $product->fragrance_type) == 'Fresh Spicy') ? 'selected' : '' }}>Fresh Spicy</option>
                            <option value="Oriental Vanilla" {{ (old('fragrance_type', $product->fragrance_type) == 'Oriental Vanilla') ? 'selected' : '' }}>Oriental Vanilla
                            </option>
                            <option value="Aromatic Fougere" {{ (old('fragrance_type', $product->fragrance_type) == 'Aromatic Fougere') ? 'selected' : '' }}>Aromatic Fougere
                            </option>
                            <option value="Citrus" {{ (old('fragrance_type', $product->fragrance_type) == 'Citrus') ? 'selected' : '' }}>Citrus</option>
                            <option value="Oriental Floral" {{ (old('fragrance_type', $product->fragrance_type) == 'Oriental Floral') ? 'selected' : '' }}>Oriental Floral
                            </option>
                            <option value="Fruity Chypre" {{ (old('fragrance_type', $product->fragrance_type) == 'Fruity Chypre') ? 'selected' : '' }}>Fruity Chypre</option>
                            <option value="Aldehydic Floral" {{ (old('fragrance_type', $product->fragrance_type) == 'Aldehydic Floral') ? 'selected' : '' }}>Aldehydic Floral
                            </option>
                            <option value="Woody Spicy" {{ (old('fragrance_type', $product->fragrance_type) == 'Woody Spicy') ? 'selected' : '' }}>Woody Spicy</option>
                            <option value="Spicy Leather" {{ (old('fragrance_type', $product->fragrance_type) == 'Spicy Leather') ? 'selected' : '' }}>Spicy Leather</option>
                            <option value="Chypre Fruity" {{ (old('fragrance_type', $product->fragrance_type) == 'Chypre Fruity') ? 'selected' : '' }}>Chypre Fruity</option>
                    ...
                    <div>
                        ...
                        <select id="size" name="size" required
                            ...
                            <option value="30ml" {{ (old('size', $product->size) == '30ml') ? 'selected' : '' }}>30ml
                            </option>
                            <option value="50ml" {{ (old('size', $product->size) == '50ml') ? 'selected' : '' }}>50ml
                            </option>
                            <option value="75ml" {{ (old('size', $product->size) == '75ml') ? 'selected' : '' }}>75ml
                            </option>
                            <option value="100ml" {{ (old('size', $product->size) == '100ml') ? 'selected' : '' }}>100ml
                            </option>
                            <option value="150ml" {{ (old('size', $product->size) == '150ml') ? 'selected' : '' }}>150ml
                            </option>
                        </select>
                        @error('size')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" required
                            ...
                            <option value="M" {{ (old('gender', $product->gender) == 'M') ? 'selected' : '' }}>Men
                            </option>
                            <option value="F" {{ (old('gender', $product->gender) == 'F') ? 'selected' : '' }}>
                                Female</option>
                            <option value="U" {{ (old('gender', $product->gender) == 'U') ? 'selected' : '' }}>
                                Unisex</option>
                        </select>
                    ...
                    <div class="flex items-end">
                        <div class="checkbox-container flex items-center mt-2">
                            ...
                            <input type="checkbox" id="bestseller" name="bestseller" value="1" {{ old('bestseller', $product->Bestseller) ? 'checked' : '' }}>
                ...
                <div class="mb-8">
                    ...
                    <textarea id="product_desc" name="product_desc" rows="5" required
                        class="input-field w-full px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold">{{ old('product_desc', $product->product_desc) }}</textarea>
                    @error('product_desc')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-8">
                    ...
                    @if($product->images->count() > 0)
                        <div class="mb-6">
                            <p class="text-sm text-gray-600 mb-3">Current Images (Uploading new images will replace these):
                            </p>
                                ...
                                @foreach($product->images as $image)
                                    <div class="relative group">
                                        ...
                                        @if($image->is_featured)
                                            <span
                                                class="absolute top-2 right-2 bg-gold text-white text-xs px-2 py-1 rounded">Featured</span>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <p class="text-sm text-gray-600 mb-3">Upload new images to replace existing ones:</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div
                    ...
                    <p class="text-sm text-gray-500 mt-2">First image will be set as featured. Uploading new images will
